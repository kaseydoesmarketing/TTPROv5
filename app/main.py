...
def _set_session_cookie(resp: JSONResponse, req: Request, raw_session_token: str, days: int = 7):
	cookie_domain = _cookie_domain_for_request(req)
...
def _clear_session_cookie(resp: JSONResponse, req: Request):
	cookie_domain = _cookie_domain_for_request(req)
	resp.delete_cookie(key=SESSION_COOKIE_NAME, domain=cookie_domain)
...
def auth_login(request: Request, db: Session = Depends(get_db)):
	...
	user.session_token = hashlib.sha256(raw_session.encode()).hexdigest()
	from datetime import timezone as _tz
	user.session_expires = datetime.now(_tz.utc) + timedelta(days=7)
	db.commit()
	resp = JSONResponse({"ok": True, "user": {"id": user.id, "email": user.email, "display_name": user.display_name}})
	_set_session_cookie(resp, request, raw_session)
	return resp

@auth_router.post("/logout")
def auth_logout(request: Request, current=Depends(get_current_user_session), db: Session = Depends(get_db)):
...
async def get_campaign_kpis(current_user: User = Depends(get_current_user_session), db: Session = Depends(get_db)):
	"""Get campaign KPIs"""
	try:
		from .models import ABTest
...
async def get_campaign_activity(current_user: User = Depends(get_current_user_session), db: Session = Depends(get_db)):
...
async def get_youtube_channels(current_user: User = Depends(get_current_user_session), db: Session = Depends(get_db)):
...
async def create_billing_portal(current_user: User = Depends(get_current_user_session)):
...
async def get_quota_usage(current_user: User = Depends(get_current_user_session), db: Session = Depends(get_db)):
...
@app.get("/auth/me")
def get_current_user_info(current_user: User = Depends(get_current_user_session)):
...
@app.get("/protected")
def protected_route(current_user: User = Depends(get_current_user_session)):
